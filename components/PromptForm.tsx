...
interface PromptFormProps {
  onGenerate: (prompt: string, aspectRatio: AspectRatio, referenceImage?: {data: string, mimeType: string}) => void;
...
const PromptForm: React.FC<PromptFormProps> = ({ onGenerate, onSave, isLoading, currentPrompt, setCurrentPrompt }) => {
  ...
  const handleGenerateClick = async () => {
    if (currentPrompt.trim() || referenceImage.file) {
      ...
      onGenerate(currentPrompt, aspectRatio, imagePayload);
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-base-100 p-6 rounded-2xl shadow-lg">
      <h2 className="text-xl font-bold text-base-content mb-4">Image Generator</h2>
      ...
      <div className="space-y-4">
        <textarea
          ...
          className="w-full h-28 p-3 bg-base-200 border border-base-300 rounded-lg focus:ring-2 focus:ring-brand-primary focus:border-brand-primary transition duration-200"
          disabled={isLoading}
        />
        
        <div>
          <label className="block text-sm font-medium text-base-content mb-2">Reference Image (Optional)</label>
          {referenceImage.preview ? (
            ...
            <label className="flex justify-center w-full h-32 px-4 transition bg-base-200 border-2 border-base-300 border-dashed rounded-md appearance-none cursor-pointer hover:border-brand-primary focus:outline-none">
              <span className="flex items-center space-x-2">
                <UploadIcon className="w-6 h-6 text-base-content-secondary" />
                <span className="font-medium text-base-content-secondary">
        ...
        <div>
          <label htmlFor="aspect-ratio" className="block text-sm font-medium text-base-content mb-1">Aspect Ratio</label>
          <select
            id="aspect-ratio"
            value={aspectRatio}
            onChange={(e) => setAspectRatio(e.target.value as AspectRatio)}
            className="w-full p-2 bg-base-200 border border-base-300 rounded-lg focus:ring-2 focus:ring-brand-primary focus:border-brand-primary transition duration-200 disabled:opacity-50 disabled:bg-base-300"
            disabled={isLoading || !!referenceImage.preview}
          >
            {ASPECT_RATIOS.map(ratio => <option key={ratio} value={ratio}>{ratio}</option>)}
          </select>
           {referenceImage.preview && <p className="text-xs text-base-content-secondary mt-1">Aspect ratio is determined by the reference image.</p>}
        </div>
      ...
      {showSaveOptions && !isLoading && currentPrompt.trim() && (
        <div className="mt-6 pt-6 border-t border-base-300 space-y-4 animate-fade-in">
          <h3 className="font-semibold text-base-content">Save This Prompt</h3>
          <input
            type="text"
            value={promptName}
            onChange={(e) => setPromptName(e.target.value)}
            placeholder="Prompt Name (e.g., Futuristic Lion King)"
            className="w-full p-2 bg-base-200 border border-base-300 rounded-lg focus:ring-2 focus:ring-brand-primary focus:border-brand-primary"
          />
          <input
            type="text"
            value={category}
